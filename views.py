...
class View:
    ...
    def display_choices(self, choices, products_importer=False):
        ...
        return choices[:-1] if products_importer and int(choice) == len(choices) else choices[int(choice) - 1]

    def menu_screen(self):
        print("\n---------------------------------------------------------------")
        print(self.text_color.green_text("WELCOME {}!".format(self.user.username) if self.user else "WELCOME"))
        print("---------------------------------------------------------------\n")
        if self.user:
            self.change_state(self.display_choices(self.logged_in_menu_choices))
        else:
            self.change_state(self.display_choices(self.logged_out_menu_choices))
    ...
    def login_screen(self):
        ...
        while not user_is_authenticated:
            ...
            user_authentication = self.controller.authenticate_user(username=username_input, password=password_input)
    ...
    def register_screen(self):
        ...
        self.controller.add_user(user=self.controller.generate_user(username=username_input, password=password_input))
        print(self.text_color.green_text("\nYou successfully created an account, you can now log in"))
        input(self.text_color.blue_text("\nPress ENTER to get back to the main menu\n"))
        self.change_state("Menu")
    ...
    def replace_product_screen(self):
        ...
        print("---------------------------------------------------------------\n")
        products = self.controller.get_products_from_bdd([self.display_choices(self.controller.get_categories())])
        choices = [product["id"] for product in products]
        choice = 0
        self.display_products(products)
        while choice not in choices:
            choice = int(input(self.text_color.blue_text("\nSelect a product by its Id, to get healthier options : ")))
        ...
        healthier_products = self.controller.get_healthier_products(product_to_replace)
            ...
            if choice == "1" and not self.user:
                print(
                    self.text_color.red_text(
                        ...
                        "home page"))
        if choice == "1":
            self.controller.save_replaced_product(self.controller.generate_SavedProduct(self.user, product_to_replace, healthier_selected_product))
            print(self.text_color.green_text("\nYou successfully saved this product replacement!\n"))
        input(self.text_color.blue_text("\nPress ENTER to get back to the main menu\n"))
        self.change_state("Menu")

    def categories_list_screen(self):
        """ This function display a list of the categories stored in the database """
        print("\n---------------------------------------------------------------")
    ...
    def products_importer_screen(self):
        ...
        products = self.controller.generate_products(self.display_choices(self.controller.get_categories(), products_importer=True))
        print(self.text_color.green_text("\nImporting products...\n"))
    ...
    def saved_products_screen(self):
        ...
        for product in saved_products:
            print(
                "\033[93m {:<20} | {:<50} | {:<50} | {:<25} \033[94m| {:<50} | {:<50} | {:<50} | {:<50}\033[0m".format(
    ...
    def display_products(products):
        """
            This function displays a specified list of products as a table
                products : list
        """
        print("{:<8} | {:<80} | {:<50} | {:<50} | {:<10} | {:<30}".format("Id", "Name", "Brand", "Retailers",
                                                                          "Nutriscore",
                                                                          "Url"))
        print("------------------------------------------------------------------"
              "--------------------------------------------------------------------"
        ...
        for product in products:
            print("{:<8} | {:<80} | {:<50} | {:<50} | {:<10} | {:<30}".format(product["id"], product["name"],
                                                                              product["brand"],
                                                                              product["retailer"],
                                                                              int(product["nutriscore"]),
                                                                              product["url"]))